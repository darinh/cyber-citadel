"""Procedural sound-design library (numpy) - license-safe, self-contained cue assets.

Subtle, tasteful cues: one whoosh per transition, a deep boom on section/title beats,
a chime/riser on quizzes, a low rumble when an antagonist speaks, a sting on persona
intros, a soft click on cards. Rendered ONCE to engine/assets/sfx/*.wav (bundled, so
nothing is generated per course) and overlaid (ducked) by the assembler. Music, by
contrast, is always SOURCED public-domain/CC (see engine/music.py) — never generated.
"""
...
SCENE_SFX = {
    ...
    "guardian": ("sting", -13), "persona": ("sting", -13),
    "control": ("click", -16), "concept": ("click", -16), "quote": ("sparkle", -16),
