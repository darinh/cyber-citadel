...
PROJECT = Path(os.environ.get("CC_PROJECT") or Path.cwd())   # the course being built (cwd)
FPS = 30
ASR = 24000          # narration sample rate
MSR = 48000          # final mix sample rate
LEAD, GAP, TAIL = 0.35, 0.30, 0.7
QUIZ_THINK = 6.0     # seconds of think time during the quiz question phase
REVEAL_HOLD = 3.6
_T = _theme.load()
# ffmpeg drawtext needs an escaped path to the vendored/themed bold font (quiz countdown)
FONTB = _theme.font_path("bold", _T).replace("\\", "/").replace(":", "\\:")
# fonts dir for libass so burned captions use the vendored font cross-platform
FONTS_DIR_ESC = str(_theme.FONT_DIR).replace("\\", "/").replace(":", "\\:")


def _ass_color(tok):
    """A palette token or #rrggbb -> ASS &H00BBGGRR."""
    if isinstance(tok, str) and tok.startswith("#") and len(tok) == 7:
        r, g, b = tok[1:3], tok[3:5], tok[5:7]
        return f"&H00{b}{g}{r}".upper()
    r, g, b = _theme.color(tok, _T)
    return f"&H00{b:02X}{g:02X}{r:02X}"


# Caption color per speaker: each cast member's `caption_color` (token/#hex), else a
# palette rotation. NARRATOR defaults to ink. No course-specific names are hardcoded.
_ROT = ["accent", "gold", "mint", "accent2", "violet"]
SPEAKER_COLOR = {"NARRATOR": _ass_color("ink")}
for _i, _cm in enumerate(_T.get("cast", []) or []):
    _nm = (_cm.get("name") or "").upper()
    if _nm:
        SPEAKER_COLOR[_nm] = _ass_color(_cm.get("caption_color", _ROT[_i % len(_ROT)]))

# Antagonist speakers (low rumble + stakes-meter dip) come from the theme cast roles.
...
_SFX = {}
for p in (ENGINE / "assets" / "sfx").glob("*.wav"):
...
Style: Cap,Noto Sans,58,&H00F5ECE8,&H00FFFFFF,&H00140A06,&H96000000,-1,0,0,0,100,100,0,0,1,4,1,2,240,240,56,1
...
def write_captions(cues, ass_path, srt_path):
    ...
    for a, b, tx, sp in cues:
        col = SPEAKER_COLOR.get(sp, _ass_color("ink"))
        ital = "\\i1" if sp in ANTAGONISTS else ""
...
def assemble(spec_path, limit=None):
    ...
    bgp = (Path(_bg) if (_bg and Path(_bg).is_absolute())
           else (PROJECT / "assets" / "backgrounds" / f"{_bg}.png") if _bg else None)
    scene.set_background(str(bgp) if (bgp and bgp.exists()) else None)
    bdir = PROJECT / ".cache" / "build" / epid             # scratch (concat lists, music, final mux)
    if bdir.exists():
    ...
    musicwav = bdir / "music.wav"; music.make_bed(T + 0.5, str(musicwav), epid=epid, spec=spec)
    sfxbed = bdir / "sfx.wav"; build_sfx_bed(sfx_cues, T, sfxbed)

    out_dir = PROJECT / "course" / "episodes"; out_dir.mkdir(parents=True, exist_ok=True)
    ...
    video_fc = (f"[0:v]fade=t=in:st=0:d=0.6,fade=t=out:st={T-0.8:.2f}:d=0.8[vb];"
                + av_chain + f"{prev}{grade}[grd];[grd]subtitles=ep.ass:fontsdir='{FONTS_DIR_ESC}'[v]")
